...
module CAO::MACRO
  class Scene_Base
    ...
    def create_help_window
      return unless @params[:help_text]
      return unless @main_window
      return unless @main_window.respond_to?("help_window=")
      return if @main_window.help_window
      return if @help_window
      @help_window = Window_Help.new
      @main_window.define_singleton_method(
        :update_help, -> { help_window.set_text(help_text) })
      @main_window.help_window = @help_window
      set_viewport(@help_window)
    end
    #------------------------------------------------------------------------
    # ● ビューポートの設定
    #------------------------------------------------------------------------
    def set_viewport(window)
    ...
    def adjust_all_windows
      instance_variables.each do |varname|
        ...
        set_viewport(ivar) if ivar.is_a?(Window)
      end
    end
    #------------------------------------------------------------------------
    # ● 全ウィンドウの更新
    ...
    # ● デフォルトのハンドラを設定する
    #------------------------------------------------------------------------
    def set_template_handler
      return unless @main_window
      unless @main_window.handle?(:ok)
        if @params[:command]
          @main_window.set_handler(:ok, method(:on_command))
        else
          @main_window.set_handler(:ok, method(:on_ok))
        end
      end
      unless @main_window.handle?(:cancel)
        @main_window.set_handler(:cancel, method(:on_cancel))
      end
    end
    #------------------------------------------------------------------------
    # ● 決定されたとき
    #------------------------------------------------------------------------
    def on_ok
      self.result = (@main_window) ? @main_window.result : nil
      return_scene
    end
    #------------------------------------------------------------------------
    # ● 決定されたとき
    #------------------------------------------------------------------------
    def on_command
      self.result = (@main_window) ? call_command(@main_window.result) : nil
      return_scene
    end
    #------------------------------------------------------------------------
    # ● キャンセルされたとき
...
module CAO::MACRO
  ...
  class Scene_Command < Scene_Base
    ...
    def start
      super
      @main_window = Window_Command.new(@params)
      adjust_all_windows
    end
    #------------------------------------------------------------------------
    # ● 決定
    #------------------------------------------------------------------------
    def on_command
      call_command(@main_window.result)
...
module CAO::MACRO
  ...
  class Scene_Number < Scene_Base
    ...
    def start
      super
      @main_window = Window_Number.new(@params)
      adjust_all_windows
    end
  end
end
...
module CAO::MACRO
  class Window_List < ::Window_Selectable
    ...
    def index=(value)
      ...
      unless over_cursor?
        @index = (@index > last_id) ? last_id : @index
      end
    ...
    #------------------------------------------------------------------------
    def item_max
      @commands.empty? ? @data.size : @commands.size
    end
    #------------------------------------------------------------------------
    # ● 項目の幅を取得
    #------------------------------------------------------------------------
    ...
    def current_item_enabled?
      ...
      (over_cursor? || start_id <= index && index <= last_id)
    end
    #------------------------------------------------------------------------
    # ● コマンドの有効状態を取得
    #------------------------------------------------------------------------
    def command_enabled?(id)
      true
    end
    #------------------------------------------------------------------------
    # ● 全項目の描画
    ...
    def draw_item(index)
      ...
      change_color(normal_color, command_enabled?(id))
  ...
  class Scene_List < Scene_Base
    ...
    def start
      super
      @main_window = Window_List.new(@params)
      @main_window.set_handler(:btn_x, method(:on_btn_x))
      @main_window.set_handler(:btn_y, method(:on_btn_y))
      adjust_all_windows
    end
    #------------------------------------------------------------------------
    # ● 決定
    ...
    #------------------------------------------------------------------------
    def on_command
      call_command(@main_window.index, @main_window.result)
      @main_window.activate
    end
    #------------------------------------------------------------------------
    # ● X ボタン (カーソルリセット)
    #------------------------------------------------------------------------
    def on_btn_x
      @main_window.cursor_switch
    end
    #------------------------------------------------------------------------
    # ● Y ボタン (検索)
    #------------------------------------------------------------------------
    def on_btn_y
      @main_window.instance_eval do
